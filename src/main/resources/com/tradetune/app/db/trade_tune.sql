DROP
DATABASE IF EXISTS trade_tune;
CREATE
DATABASE IF NOT EXISTS trade_tune
...
INSERT INTO vehicle_category (id_category, name)
VALUES (1, 'CAR'),
       (2, 'MOTORCYCLE'),
       (3, 'MOPED');
...
INSERT INTO vehicle_status (id_vehicle_status, name)
VALUES (1, 'IN_STOCK'),
       (2, 'SOLD'),
       (3, 'IN_REPAIR');
...
INSERT INTO payment_method (id_payment_method, name)
VALUES (1, 'CASH'),
       (2, 'CARD'),
       (3, 'TRANSFER'),
       (4, 'FINANCING');
...
INSERT INTO proposal_status (id_proposal_status, name)
VALUES (1, 'ACTIVE'),
       (2, 'ACCEPTED'),
       (3, 'REJECTED');
...
INSERT INTO repair_status (id_repair_status, name)
VALUES (1, 'PENDING'),
       (2, 'ASSIGNED'),
       (3, 'IN_PROGRESS'),
       (4, 'FINISHED'),
...
CREATE TRIGGER trg_worker_activate_head_mechanic
    ...
    FOR EACH ROW
BEGIN
    DECLARE is_head_mechanic INT;
    DECLARE active_count INT;

    IF OLD.active = 0 AND NEW.active = 1 THEN

    SELECT COUNT(*)
    INTO is_head_mechanic
    FROM head_mechanic
    WHERE id_worker = NEW.id_worker;
...
BEGIN
    ...
    IF (
        (NEW.assigned_mechanic <> OLD.assigned_mechanic)
...
-- 1) Concesionarios
-- =========================
INSERT INTO dealership (name, address, city, province, country)
VALUES ('Trade & Tune Málaga', 'Av. de Velázquez 284', 'Málaga', 'Málaga', 'España'),
...
VALUES (1, 'laura.garcia@trade-tune.es', '$2b$10$malaga.manager.hash.demo', 'Laura García Ramos', '+34 600 123 456',
        '2022-03-01', 1),
       (1, 'daniel.ruiz@trade-tune.es', '$2b$10$malaga.sales1.hash.demo', 'Daniel Ruiz Ortega', '+34 611 222 333',
        '2023-01-15', 1),
       (1, 'marta.lopez@trade-tune.es', '$2b$10$malaga.sales2.hash.demo', 'Marta López Navarro', '+34 612 444 555',
        '2023-06-12', 1),
       (1, 'ivan.moreno@trade-tune.es', '$2b$10$malaga.mech1.hash.demo', 'Iván Moreno Gil', '+34 613 666 777',
        '2021-09-20', 1),
       (1, 'paula.suarez@trade-tune.es', '$2b$10$malaga.mech2.hash.demo', 'Paula Suárez Castro', '+34 614 888 999',
        '2024-02-05', 1),
       (1, 'carlos.perez@trade-tune.es', '$2b$10$malaga.head.hash.demo', 'Carlos Pérez Molina', '+34 615 101 202',
        '2020-11-10', 1),

       (1, 'sales1@demo.com', '1234', 'Sales One', '+34 600 000 011', '2026-03-02', 1),
       (1, 'sales2@demo.com', '1234', 'Sales Two', '+34 600 000 012', '2026-03-02', 1),
       (1, 'mech1@demo.com', '1234', 'Mech One', '+34 600 000 013', '2026-03-02', 1),
       (1, 'mech2@demo.com', '1234', 'Mech Two', '+34 600 000 014', '2026-03-02', 1),

       (2, 'ana.martin@trade-tune.es', '$2b$10$sevilla.manager.hash.demo', 'Ana Martín Jiménez', '+34 620 303 404',
        '2022-05-03', 1),
       (2, 'javier.romero@trade-tune.es', '$2b$10$sevilla.sales.hash.demo', 'Javier Romero León', '+34 621 505 606',
        '2023-10-01', 1),
       (2, 'lucas.vargas@trade-tune.es', '$2b$10$sevilla.mech1.hash.demo', 'Lucas Vargas Herrera', '+34 622 707 808',
        '2021-07-19', 1),
       (2, 'sara.nieto@trade-tune.es', '$2b$10$sevilla.head.hash.demo', 'Sara Nieto Fuentes', '+34 623 909 010',
        '2020-04-08', 1);

-- Managers
INSERT INTO manager (id_worker)
SELECT id_worker
FROM worker
WHERE email IN ('laura.garcia@trade-tune.es', 'ana.martin@trade-tune.es');

-- Sales employees (incluye tus demos si quieres)
INSERT INTO sales_employee (id_worker)
SELECT id_worker
FROM worker
WHERE email IN ('daniel.ruiz@trade-tune.es', 'marta.lopez@trade-tune.es', 'javier.romero@trade-tune.es',
                'sales1@demo.com', 'sales2@demo.com');

-- Mechanics
INSERT INTO mechanic (id_worker)
SELECT id_worker
FROM worker
WHERE email IN ('ivan.moreno@trade-tune.es', 'paula.suarez@trade-tune.es', 'carlos.perez@trade-tune.es',
                'lucas.vargas@trade-tune.es', 'sara.nieto@trade-tune.es',
                'mech1@demo.com', 'mech2@demo.com');

-- Head mechanics
INSERT INTO head_mechanic (id_worker)
SELECT id_worker
FROM worker
WHERE email IN ('carlos.perez@trade-tune.es', 'sara.nieto@trade-tune.es');


-- Especialidades (para cumplir triggers de repair)
INSERT INTO mechanic_specialty (id_mechanic, id_category)
VALUES (4, 1), -- Iván: coches
       (5, 2), -- Paula: motos
       (6, 1),
       (6, 2), -- Carlos (jefe): coches y motos
       (9, 1), -- Lucas: coches
       (10, 1),
       (10, 3);
-- Sara (jefa): coches y ciclomotores

-- =========================
-- 3) Materiales
-- =========================
INSERT INTO material_type (id_material_type, name)
VALUES (1, 'FLUID'),
       (2, 'FILTER'),
       (3, 'BRAKES'),
       (4, 'ELECTRICAL'),
...
INSERT INTO material (id_material_type, name, price_ref, active)
VALUES (1, 'Aceite motor 5W-30 (1L)', 9.50, 1),
...
-- =========================
INSERT INTO vehicle
(id_dealership, id_category, id_vehicle_status, vin, license_plate, brand, model, year, km, fuel, base_price,
 arrival_date)
VALUES (1, 1, 3, 'VSSZZZ5FZJR000101', '5483LKG', 'SEAT', 'León 1.6 TDI', 2018, 98000, 'Diesel', 13900.00, '2025-11-20'),
       (1, 1, 2, 'WVWZZZAUZLW000202', '2197KPM', 'Volkswagen', 'Golf 1.5 TSI', 2020, 62000, 'Petrol', 18900.00,
        '2025-10-05'),
       (1, 1, 1, 'JTDBR32E0M0000303', '7731MJD', 'Toyota', 'Corolla 1.8 Hybrid', 2021, 41000, 'Hybrid', 22900.00,
        '2025-12-10'),
       ...
       (1, 1, 1, 'VF1RJA00465100606', '4328MPR', 'Renault', 'Clio TCe 90', 2019, 54000, 'Petrol', 12900.00,
        '2025-11-01'),
       (1, 3, 1, 'ZAPC2500000000707', '8061HLM', 'Piaggio', 'Zip 50', 2020, 8000, 'Petrol', 1790.00, '2025-12-22'),

       (2, 1, 3, 'VF3L3HNS0KS000808', '1049LBC', 'Peugeot', '308 1.2 PureTech', 2019, 73000, 'Petrol', 14900.00,
        '2025-10-28'),
...
INSERT INTO vehicle_image (id_vehicle, url)
VALUES (1, 'assets/images/vehicles/seat_leon_2018_1.jpg'),
...
VALUES ('12345678Z', 'Sergio Martín Aguilar', 'sergio.martin@gmail.com', '+34 640 111 222', '1992-04-17',
        'C/ Héroe de Sostoa 112, Málaga'),
       ('23456789D', 'María Sánchez Moreno', 'maria.sanchez@gmail.com', '+34 640 222 333', '1989-09-02',
        'C/ Larios 14, Málaga'),
       ('34567890V', 'David López Castillo', 'david.lopez@gmail.com', '+34 640 333 444', '1996-01-25',
        'Av. Juan XXIII 45, Málaga'),
       ('45678901G', 'Lucía Fernández Ruiz', 'lucia.fernandez@gmail.com', '+34 640 444 555', '1994-12-11',
        'C/ Pacífico 67, Málaga'),
       ('56789012B', 'Raúl Ortega Torres', 'raul.ortega@gmail.com', '+34 640 555 666', '1987-06-30',
        'C/ Granada 8, Málaga'),
       ('67890123B', 'Elena Navarro Díaz', 'elena.navarro@gmail.com', '+34 640 666 777', '1990-03-08',
        'C/ San Jacinto 20, Sevilla'),
       ('78901234X', 'Adrián Molina Romero', 'adrian.molina@gmail.com', '+34 640 777 888', '1998-10-19',
        'Av. Andalucía 210, Málaga'),
       ('89012345E', 'Carmen Jiménez Herrera', 'carmen.jimenez@gmail.com', '+34 640 888 999', '1985-02-14',
        'C/ Carretería 29, Málaga'),
       ('90123456A', 'Pablo Vargas Núñez', 'pablo.vargas@gmail.com', '+34 641 000 111', '1991-07-07',
        'C/ Mármoles 5, Málaga'),
       ('11223344B', 'Noelia Gil Rojas', 'noelia.gil@gmail.com', '+34 641 111 222', '1999-05-21',
        'C/ Alcazabilla 3, Málaga'),
       ('22334455Y', 'Héctor León Campos', 'hector.leon@gmail.com', '+34 641 222 333', '1993-08-09',
        'C/ Nervión 12, Sevilla'),
       ('33445566R', 'Isabel Fuentes Blanco', 'isabel.fuentes@gmail.com', '+34 641 333 444', '1988-11-27',
        'Av. de la Palmera 38, Sevilla');

INSERT INTO client_vehicle_interest
    (id_client, id_vehicle, interest_date, approx_budget, notes)
VALUES (1, 2, '2025-12-16', 18500.00, 'Busca financiación a 48 meses.'),
...
VALUES (1, 2, 2, 4, 2, '2025-12-18', '2026-01-08', 18900.00, 900.00, 18000.00,
        'Incluye cambio de aceite antes de entrega.'),
       (2, 4, 3, 3, 2, '2025-10-01', '2025-10-21', 17900.00, 1400.00, 16500.00,
        'Transferencia y entrega con ITV en vigor.'),
       (3, 3, 2, 2, 3, '2026-01-10', '2026-01-25', 22900.00, 2500.00, 20400.00,
        'Cliente solicita descuento extra; no aceptado.'),
       (4, 6, 3, 2, 1, '2026-02-01', '2026-02-20', 12900.00, 500.00, 12400.00, 'Oferta activa pendiente de visita.'),
       (6, 9, 8, 3, 1, '2026-01-20', '2026-02-15', 29900.00, 0.00, 29900.00,
        'Oferta activa: incluye gestión de transferencia.'),
       (5, 10, 8, 2, 2, '2025-12-05', '2025-12-25', 5390.00, 200.00, 5190.00,
        'Entrega con revisión general y limpieza.');

INSERT INTO sale
    (id_proposal, id_vehicle, sale_ts, final_price_snapshot)
VALUES (1, 2, '2025-12-22 12:15:00', 18000.00),
       (2, 4, '2025-10-05 10:30:00', 16500.00),
       (6, 10, '2025-12-10 18:40:00', 5190.00);


-- =========================
-- 6.1) Especialidades de mecánicos (ANTES de repairs)
-- =========================
-- Iván: le damos las categorías de los vehículos 1,6,7 (los que le asignas en repairs)
INSERT IGNORE INTO mechanic_specialty (id_mechanic, id_category)
SELECT m.id_worker, v.id_category
FROM mechanic m
         JOIN worker w ON w.id_worker = m.id_worker
         JOIN vehicle v ON v.id_vehicle IN (1, 6, 7)
WHERE w.email = 'ivan.moreno@trade-tune.es'
GROUP BY m.id_worker, v.id_category;

-- Paula: categoría del vehículo 5 (el que le asignas en repairs)
INSERT IGNORE INTO mechanic_specialty (id_mechanic, id_category)
SELECT m.id_worker, v.id_category
FROM mechanic m
         JOIN worker w ON w.id_worker = m.id_worker
         JOIN vehicle v ON v.id_vehicle = 5
WHERE w.email = 'paula.suarez@trade-tune.es'
GROUP BY m.id_worker, v.id_category;

-- Lucas: categoría del vehículo 8 (repair de dealership 2)
INSERT IGNORE INTO mechanic_specialty (id_mechanic, id_category)
SELECT m.id_worker, v.id_category
FROM mechanic m
         JOIN worker w ON w.id_worker = m.id_worker
         JOIN vehicle v ON v.id_vehicle = 8
WHERE w.email = 'lucas.vargas@trade-tune.es'
GROUP BY m.id_worker, v.id_category;

-- =========================
-- 7) Reparaciones (1 INSERT, ids por email)
-- =========================
INSERT INTO repair
...
VALUES (1, 1, NULL,
        (SELECT hm.id_worker
         ...
         WHERE w.email = 'carlos.perez@trade-tune.es'),
        (SELECT m.id_worker
         FROM mechanic m
                  JOIN worker w ON w.id_worker = m.id_worker
         WHERE w.email = 'ivan.moreno@trade-tune.es'),
        3, '2026-01-25 09:30:00', '2026-01-26 10:00:00', NULL, 6.50, 420.00, NULL,
       ...
       (1, 5, 5,
        ...
        (SELECT m.id_worker
         FROM mechanic m
                  JOIN worker w ON w.id_worker = m.id_worker
         WHERE w.email = 'paula.suarez@trade-tune.es'),
        2, '2026-01-28 11:10:00', NULL, NULL, 1.50, 95.00, NULL,
       ...
       (1, 6, 8,
        ...
        (SELECT m.id_worker
         FROM mechanic m
                  JOIN worker w ON w.id_worker = m.id_worker
         WHERE w.email = 'ivan.moreno@trade-tune.es'),
        4, '2025-11-10 08:45:00', '2025-11-10 10:00:00', '2025-11-10 15:30:00', 2.50, 160.00, 155.00,
       ...
       (2, 8, NULL,
        (SELECT hm.id_worker
         FROM head_mechanic hm
                  JOIN worker w ON w.id_worker = hm.id_worker
         WHERE w.email = 'sara.nieto@trade-tune.es'),
        (SELECT m.id_worker
         FROM mechanic m
                  JOIN worker w ON w.id_worker = m.id_worker
         WHERE w.email = 'lucas.vargas@trade-tune.es'),
        2, '2026-01-15 16:20:00', NULL, NULL, 5.00, 380.00, NULL,
       ...
       (1, 7, 9,
        (SELECT hm.id_worker
         ...
         WHERE w.email = 'carlos.perez@trade-tune.es'),
        NULL,
        1, '2026-02-02 09:00:00', NULL, NULL, 0.75, 50.00, NULL,
        'Diagnóstico eléctrico (arranque)', 'No arranca en frío; revisar batería y relé de arranque.');

-- =========================
-- 7b) Materiales usados (1 INSERT, id_repair por vehicle+creation_ts)
-- =========================
INSERT INTO repair_material (id_repair, id_material, quantity, unit_price_applied)
VALUES
-- Repair 1 (Seat León) -> vehicle=1, creation_ts='2026-01-25 09:30:00'
((SELECT r.id_repair
  FROM repair r
  WHERE r.id_vehicle = 1 AND r.creation_ts = '2026-01-25 09:30:00' LIMIT 1), 8, 1, 195.00),
((SELECT r.id_repair FROM repair r WHERE r.id_vehicle=1 AND r.creation_ts='2026-01-25 09:30:00' LIMIT 1),  1, 4,   9.50),
((SELECT r.id_repair FROM repair r WHERE r.id_vehicle=1 AND r.creation_ts='2026-01-25 09:30:00' LIMIT 1),  2, 1,  12.90),

-- Repair 2 (MT-07) -> vehicle=5, creation_ts='2026-01-28 11:10:00'
((SELECT r.id_repair FROM repair r WHERE r.id_vehicle=5 AND r.creation_ts='2026-01-28 11:10:00' LIMIT 1),  7, 1,  55.00),
((SELECT r.id_repair FROM repair r WHERE r.id_vehicle=5 AND r.creation_ts='2026-01-28 11:10:00' LIMIT 1),  5, 1,   8.50),

-- Repair 3 (Clio) -> vehicle=6, creation_ts='2025-11-10 08:45:00'
((SELECT r.id_repair FROM repair r WHERE r.id_vehicle=6 AND r.creation_ts='2025-11-10 08:45:00' LIMIT 1),  1, 4,   9.50),
((SELECT r.id_repair FROM repair r WHERE r.id_vehicle=6 AND r.creation_ts='2025-11-10 08:45:00' LIMIT 1),  2, 1,  12.90),
((SELECT r.id_repair FROM repair r WHERE r.id_vehicle=6 AND r.creation_ts='2025-11-10 08:45:00' LIMIT 1),  3, 1,  18.50),
((SELECT r.id_repair FROM repair r WHERE r.id_vehicle=6 AND r.creation_ts='2025-11-10 08:45:00' LIMIT 1),  4, 1,  16.90),

-- Repair 4 (308) -> vehicle=8, creation_ts='2026-01-15 16:20:00'
((SELECT r.id_repair FROM repair r WHERE r.id_vehicle=8 AND r.creation_ts='2026-01-15 16:20:00' LIMIT 1),  9, 1, 210.00),
((SELECT r.id_repair FROM repair r WHERE r.id_vehicle=8 AND r.creation_ts='2026-01-15 16:20:00' LIMIT 1), 14, 2,   7.00),

-- Repair 5 (Zip 50) -> vehicle=7, creation_ts='2026-02-02 09:00:00'
((SELECT r.id_repair FROM repair r WHERE r.id_vehicle=7 AND r.creation_ts='2026-02-02 09:00:00' LIMIT 1), 11, 1, 115.00);
